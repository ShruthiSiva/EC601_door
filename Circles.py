...
def outline(original_img, maxValue = 255, blockSize = 7, C = 3, g_blur = 10):
    ...
    img2 = cv2.adaptiveThreshold(img,maxValue,threshold,cv2.THRESH_BINARY,blockSize,C)
...
def findBindingBox(original_img):
    ...
    temp,contours, hierarchy = cv2.findContours(temp,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
